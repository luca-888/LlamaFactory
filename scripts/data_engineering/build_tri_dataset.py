#!/usr/bin/env python3
"""Build tri-label SFT dataset with independent pipeline."""
...
from common import normalize_behavior_label, normalize_dynamic_events, normalize_static_labels, valid_session_id
...
def build_record(raw: dict[str, Any], stats: Counter[str], prompt: dict[str, Any]) -> dict[str, Any] | None:
    ...
    static_labels = normalize_static_labels(raw.get("static_metadata"), stats)
    if static_labels is None:
        stats["invalid_static_metadata"] += 1
        return None

    dynamic_events = normalize_dynamic_events(raw.get("dynamic_metadata"), stats)
    if dynamic_events is None:
        stats["invalid_dynamic_metadata"] += 1
        return None
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Build tri-label dataset")
    ...
    if args.list_prompt_versions:
        for v in list_prompt_versions("tri"):
            print(v)
        return
    ...
    prompt = load_prompt_config("tri", args.prompt_version)
